...
function calculateOBVTrend(data) {
    if (!data || data.length < 30 || !data[0].obv) return { direction: 'neutral', strength: 0 };
    
    const recentData = data.slice(-20);
    const firstPoint = recentData[0].obv;
    const lastPoint = recentData[recentData.length - 1].obv;
    
    const changePercent = ((lastPoint - firstPoint) / Math.abs(firstPoint)) * 100;
...
function initializeStockChart(symbol, containerId) {
    ...
    try {
        ...
        const stock = accumulationStocks.find(s => s.symbol === symbol);
        const supportLevel = stock && stock.details.wyckoff && stock.details.wyckoff.keyLevels ? 
            stock.details.wyckoff.keyLevels.supportLevel : null;
        const resistanceLevel = stock && stock.details.wyckoff && stock.details.wyckoff.keyLevels ? 
            stock.details.wyckoff.keyLevels.resistanceLevel : null;
        ...
        // Y scale
        const minY = Math.min(
            d3.min(displayData, d => d.low) * 0.995,
            supportLevel ? supportLevel * 0.995 : Infinity
        );
        const maxY = Math.max(
            d3.max(displayData, d => d.high) * 1.005,
            resistanceLevel ? resistanceLevel * 1.005 : -Infinity
        );
...
function refreshData() {
    ...
    fetchHistoricalData().then(() => {
        ...
        loadCurrentPrices().then(() => {
            // And finally detect patterns with the updated data
            detectAccumulationPatterns();
            ...
            console.error("Error loading current prices:", error);
            // Still try to detect patterns with available data
            detectAccumulationPatterns();
